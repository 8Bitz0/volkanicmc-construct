...
impl Template {
    pub async fn import(file: path::PathBuf) -> Result<Self, ParseError> {
        let template = parse::json_to_template(file).await?;

        for r in &template.resources {
            if let resource::GenericResource::Remote {
                url: _,
                user_agent: _,
                override_name: _,
                sha512: _,
                ...
                if archive.is_some() && use_variables.is_some() {
                    return Err(ParseError::ArchiveVariables(template_path.to_string_lossy().to_string()));
                }
            }
        }
