...
pub async fn download(store: vkstore::VolkanicStore, url: &str, verification: Verification, name: path::PathBuf) -> Result<path::PathBuf, DownloadError> {
...
pub async fn extract(store: vkstore::VolkanicStore, orig_path: path::PathBuf, format: ArchiveFormat) -> Result<path::PathBuf, ExtractionError> {
    ...
    if new_path.is_dir() {
        fs::remove_dir_all(&new_path).await.map_err(ExtractionError::FilesystemError)?;
    }
    ...
    match format {
        ArchiveFormat::TarGz => {
            ...
            fs::create_dir_all(&new_path).await.map_err(ExtractionError::FilesystemError)?;

            archive.unpack(&new_path).map_err(ExtractionError::TarError)?;
        }
